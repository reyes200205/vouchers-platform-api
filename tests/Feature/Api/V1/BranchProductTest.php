...
describe('Branch products', function (): void {
    it('lets a branch manager create a product in their own branch with a category', function (): void {
    ...
    it('lets a branch manager update only products of their branch', function (): void {
        $myBranch = Branch::factory()->create();
        $otherBranch = Branch::factory()->create();
        $category = productCategory($myBranch);
        productSignIn('branch_manager', $myBranch);
        ...
        $this->patchJson("/api/v1/branches/{$myBranch->id}/products/{$theirs->id}", [
            'name' => 'No debería',
        ])->assertNotFound();
    });
